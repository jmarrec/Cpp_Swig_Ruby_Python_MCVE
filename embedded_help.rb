...
module Kernel
  ...
  # DLM: now done in embedded gem initialization section in mylib_cli.rb
  #$LOAD_PATH << EmbeddedScripting::findFirstFileByName('mylib-standards.rb').gsub('/mylib-standards.rb', '')
  #$LOAD_PATH << EmbeddedScripting::findFirstFileByName('mylib-workflow.rb').gsub('/mylib-workflow.rb', '')
  $LOADED = []
  ...
  # Consider moving this map into mylib-gems project
  ...
  def require path
    ...
    begin
      ...
      if path.include? 'mylib/energyplus/find_energyplus'
        return false
      ...
      elsif path_with_extension == 'mylib.rb'
        # Mylib is loaded by default and does not need to be required
        return true
      elsif require_embedded(path_with_extension, $LOAD_PATH)
        # Load embedded files that are no required using full paths now
        # This does not included the mylib-gems set of default, baked in gems
      ...
      # $EMBEDDED_GEM_PATH is populated in mylib_cli.rb during startup
  ...
  def require_embedded_absolute path
    ...
    s = Mylib::preprocess_ruby_script(s)
  ...
  def require_relative path
    ...
    if absolute_path.to_s.chars.first == ':'
      absolute_path = Mylib.get_absolute_path(absolute_path)
    end
  ...
  def open(name, *args)
    ...
    if name.to_s.chars.first == ':' then
      ...
      absolute_path = Mylib.get_absolute_path(name)
...
class File
  ...
  def self.expand_path(file_name, *args)
    if file_name.to_s.chars.first == ':' then
      ...
      #STDOUT.flush
      return Mylib.get_absolute_path(file_name)
      ...
      #puts "y = #{Mylib.get_absolute_path(File.join(args[0], file_name))}"
      #STDOUT.flush
      #return original_expand_path(file_name, *args)
      return Mylib.get_absolute_path(File.join(args[0], file_name))
    end
    return original_expand_path(file_name, *args)
  end
  ...
  def self.directory?(file_name)
    if file_name.to_s.chars.first == ':' then
      absolute_path = Mylib.get_absolute_path(file_name)
      EmbeddedScripting::fileNames.each do |file|
...
class Dir
  ...
  def self.glob(pattern, *args)
    ...
    pattern_array.each do |pattern|
      ...
      if pattern.to_s.chars.first == ':'
        ...
        absolute_pattern = Mylib.get_absolute_path(pattern)
  ...
  def self.chdir(dir)
    if dir.to_s.chars.first == ':'
      ...
      absolute_path = Mylib.get_absolute_path(dir)
      if block_given?
...
module FileUtils
  ...
  def self.cp_r(src, dest, options = {})
    ...
    if src.to_s.chars.first == ':' then
      absolute_path = Mylib.get_absolute_path(src)
